...
if [ "$mediawiki_git_install" = "y" ]; then
	...
	wget http://releases.wikimedia.org/mediawiki/1.28/mediawiki-core-1.28.0.tar.gz

	mkdir mediawiki
	tar xpvf mediawiki-core-1.28.0.tar.gz -C ./mediawiki --strip-components 1
	cd mediawiki
...
if [ "$mediawiki_git_install" = "y" ]; then
	...
	wget https://github.com/wikimedia/mediawiki-skins-Vector/archive/REL1_28.tar.gz
	mkdir Vector
	tar xpvf REL1_28.tar.gz -C ./Vector --strip-components 1
fi
